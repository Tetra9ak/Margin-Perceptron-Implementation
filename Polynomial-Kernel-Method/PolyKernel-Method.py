...
def Training_polykernel(X, y, w, S1, S2, max_epoch, dimension, radius, polyKernel_c):
    ...
    for index in range(max_epoch):
        ...
        violation, S1, S2, w = iteration_polykernel(X, y, w, S1, S2, dimension=dimension, gamma_guess=gamma_guess,
                                                    polyKernel_c=polyKernel_c)
        if violation > -1:
...
def Margin_Perceptron_polykernel(file_list, polyKernel_c):
    ...
    for file in file_list:
        ...
        while Training_polykernel(X, y, w, S1, S2, max_epoch=max_epochs, dimension=dimension, radius=gamma_guess,
                                  polyKernel_c=polyKernel_c):
            gamma_guess /= 2
